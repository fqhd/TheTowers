...
void HUD::renderHotbar(GUIRenderer* _renderer, Hotbar& _hotbar){
	...
	for(unsigned int i = 0; i < 11; i++) {
		...
		if(itemID != ItemID::NONE){
			_renderer->drawRect(math::vec4(414 + i * 32, 12, 64, 64), _renderer->assets.getItemTexture().getUVQuadFromItemID(itemID), _renderer->assets.getItemTexture().getTextureID(), ColorRGBA8());
		}
	}
