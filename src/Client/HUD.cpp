...
void HUD::renderHotbar(GUIRenderer* _renderer, Hotbar& _hotbar){
	...
	for(unsigned int i = 0; i < 11; i++) {
		...
		if(itemID != ItemID::NONE){
			_renderer->drawRect(math::vec4(414 + i * 32 + i * 10, 12, 32, 32), _renderer->assets.getItemTexture().getUVQuad(itemID), _renderer->assets.getItemTexture().getTextureID(), ColorRGBA8());
		}
	}
