...
void ParticleHandler::placeParticlesAroundBlock(int x, int y, int z, uint8_t _blockID){
	for(unsigned int j = 0; j < NUM_PARTICLES; j++){
		...
		m_particles.emplace_back(math::vec3(x, y, z) + math::vec3((rand()%11) / 10.0f, (rand()%11)/10.0f, (rand()%11)/10.0f), math::vec3((rand()%10) - 5, 10, (rand()%10) - 5) * 0.20f, time, 0.0f, scale, _blockID);
	}
}
